...
public class PageSurfaceView extends GLSurfaceView implements GestureDetector.OnGestureListener {
    ...
    public boolean onPageTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (Activity_PdfViewer.page_flip_mp != null)
                    Activity_PdfViewer.page_flip_mp.start();
                break;
            case MotionEvent.ACTION_MOVE:
                float perc_move = (event.getX() - x1) / (float) getWidth();
                if ((event.getX() - x1) > 0) { // page_left
                ...
                } else if ((event.getX() - x1) < 0) {//  page_right
    ...
    private void processPage() {
        if (currentPosition == 0) {
            ...
            canSwipeLeft = false;
            canSwipeRight = true;
        } else if (currentPosition == pageCurlAdapter.getCount() - 1) {
