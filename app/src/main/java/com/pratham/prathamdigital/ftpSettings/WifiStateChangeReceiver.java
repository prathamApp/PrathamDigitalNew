...
public class WifiStateChangeReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (info == null) {
            Log.e("WifiStateChangeReceiver","Null network info received, bailing");
            return;
        }
        if (info.isConnected()) {
            Log.e("WifiStateChangeReceiver","We are connecting to a wifi network");
            Intent startServerIntent = new Intent(context, StartServerService.class);
            context.startService(startServerIntent);
        } else if (info.isConnectedOrConnecting()) {
            Log.e("WifiStateChangeReceiver","Still connecting, ignoring");
        } else {
            Log.e("WifiStateChangeReceiver","We are disconnected from wifi network");
    ...
    static public class StartServerService extends IntentService {
        ...
        protected void onHandleIntent(Intent intent) {
            if (FsService.isRunning()) {
                Log.e("WifiStateChangeReceiver","We are connecting to a new wifi network on a running server, ignore");
                return;
            }
            WifiManager wifiManager = (WifiManager) getApplicationContext().getSystemService(Context.WIFI_SERVICE);
            WifiInfo wifiInfo = wifiManager.getConnectionInfo();
            if (wifiInfo == null) {
                Log.e("WifiStateChangeReceiver","Null wifi info received, bailing");
                return;
            }
            Log.e("WifiStateChangeReceiver","We are connected to " + wifiInfo.getSSID());
    ...
    static public class StopServerService extends IntentService {
        ...
        protected void onHandleIntent(Intent intent) {
            ...
            Log.e("WifiStateChangeReceiver","Wifi connection disconnected and no longer connecting, stopping the ftp server");
