...
public abstract class AutoSync {
    ...
    /**
     * Starts the {@code AutoSync} service. This is necessary for all periodic syncing to occur. The
    ...
    /**
     * Returns a proxy to a {@code AutoSync} to be able to query and modify it outside it's own
     * context. You should always use this instead of constructing the {@code AutoSync} directly.
     *
     * @param context  the context
     * @param listener the class of the {@code AutoSync} to proxy
     * @return the proxy
     */
    public static AutoSyncProxy get(Context context, Class<? extends AutoSync> listener) {
        return new AutoSyncProxy(context, listener.getName());
    }

    /**
     * Called when the {@code AutoSync} is created. This may be only called once across several
     ...
     * throw an exception to notify {@code AutoSync} to retry properly.
     *
     * @param context the context
    ...
    /**
     * Returns the current configuration of the {@code AutoSync}. This is not valid before {@link
     * #onCreate(android.content.Context)}. The returned configuration is read-only. To edit, use
     * {@link #edit(AutoSync.Edit...)} instead.
     *
    ...
    /**
     * Returns the name of the {@code AutoSync}. This is currently {@code getClass().getName()}.
     *
     * @return the name
     */
     ...
     * #edit(AutoSync.Edit...)} will take precedence. This is equivalent to setting the
     * configuration options in xml.
     *
     * @see #edit(AutoSync.Edit...)
     */
    ...
    /**
     * Class for configuring how and when a {@link AutoSync} will sync.
     */
    public static final class Config {
        ...
        private Config(Context context, String name) {
            this.name = name;
            prefs = context.getSharedPreferences(AutoSyncPreferences.NAME, Context.MODE_PRIVATE);
        }
    ...
    public static class Edit {
        ...
        /**
         * Sets if the {@link AutoSync} is enabled. If not, it will not sync, even when called
         * explicitly.
         *
         ...
         * time, AutoSync will pick a random offset from the sync interval that is within this range
