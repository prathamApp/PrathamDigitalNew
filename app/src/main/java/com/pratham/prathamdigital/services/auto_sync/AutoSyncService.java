...
import static com.pratham.prathamdigital.services.auto_sync.MathUtil.randomInRange;
...
public class AutoSyncService extends IntentService {
    ...
    private static final long MIN_RETRY_CAP = 5 * AutoSync.Config.SECONDS;
    ...
    public void onCreate() {
        ...
        prefs = new AutoSyncPreferences(this);
        seed = findOrCreateSeed(prefs);
        powerConnected = prefs.isPowerConnected();

        listeners = AutoSyncParser.parseListeners(this);
    }
    ...
    protected void onHandleIntent(Intent intent) {
        switch (intent.getIntExtra(TYPE, 0)) {
            ...
            case TYPE_SYNC: {
                ...
                AutoSync listener = listeners.get(name);
                if (listener != null) {
            ...
            case TYPE_NETWORK_BACK: {
                ...
                for (AutoSync listener : listeners.values()) {
            ...
            case TYPE_POWER_CHANGED: {
                ...
                // AutoSyncPowerReceiver is a WakefulBroadcastReceiver, so make sure to release the lock.
    ...
    private void add(AlarmManager alarmManager, AutoSync listener) {
        if (!listener.config().enabled()) return;

        AutoSync.Config config = listener.config();
    ...
    private void onHandleSync(AutoSync listener) {
        if (!listener.config().enabled()) return;
    ...
    private void onHandleSyncInexact(AutoSync listener) {
        AlarmManager alarmManager = (AlarmManager) getSystemService(Context.ALARM_SERVICE);
    ...
    private void onHandleUpdate(AutoSync listener) {
        AlarmManager alarmManager = (AlarmManager) getSystemService(Context.ALARM_SERVICE);
        remove(alarmManager, listener);
        add(alarmManager, listener);
    }

    private void onHandleFailureNoNetwork(AlarmManager alarmManager, AutoSync listener) {
        removeAll(alarmManager);
        AutoSyncNetworkReceiver.enable(this);
    }

    private void onHandleFailureSyncError(AlarmManager alarmManager, AutoSync listener) {
        AutoSync.Config config = listener.config();
        long span = config.every();
    ...
    private void remove(AlarmManager alarmManager, AutoSync listener) {
    ...
    private long findOrCreateSeed(AutoSyncPreferences prefs) {
